    ...
    if($stmt->prepare($sql)) {
        ...
        while($stmt->fetch()){
            ...
            if($_SESSION['is_superuser']){
                echo "<li><form action='php/delete_wine.php' method='post'><input type='hidden' name='wine_id' value='" . $wine_id . "'/><button class='btn btn-xs btn-danger' type='submit'>Delete Wine <span class='glyphicon glyphicon-trash' aria-hidden='true'></span></button></form></li>";
            }
            echo "</ul></a>
                    </div>";
        }
